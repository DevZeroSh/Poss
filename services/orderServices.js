...
exports.createCashOrder = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        await ReportsSalesModel.create({
            customer: req.body.customarName || "Full name",
            orderId: order._id,
            date: timeIsoString,
        ...
        console.log("orderServices 173 " + error.message);
        return new ApiError(`Error creating cash order: ${error.message}`, 500);
    }
});
...
exports.createCashOrderMultipelFunds = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        return new ApiError(`Error creating cash order multiple funds: ${error.message}`, 500);
    }
});
...
exports.editOrder = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        // Find the existing ReportsSalesModel based on orderId
        const existingReportsSales = await ReportsSalesModel.findOne({ orderId: id });

        // Update existing ReportsSalesModel with new data
        if (existingReportsSales) {
            existingReportsSales.date = new Date().toISOString();
            existingReportsSales.fund = financialFunds;
            existingReportsSales.amount = order.totalOrderPrice;
            existingReportsSales.cartItems = req.body.cartItems;
            existingReportsSales.paymentType = "Edit Order";
            existingReportsSales.employee = req.user._id;
            await existingReportsSales.save();
        }

        req.body.cartItems.map(async (item) => {
            const { _id, quantity } = await productModel.findOne({ qr: item.qr });
            createProductMovement(_id, item.quantity, quantity, "edit", "updatesales", dbName);
        });
        ...
        console.log("orderService 558 " + error.message);
        res.status(500).json({
            status: "false",
...
exports.returnOrder = asyncHandler(async (req, res, next) => {
    ...
    try {
        ...
        req.body.cartItems.map((item) => {
            createProductMovement(item.product, item.quantity, "out", "returnsales", dbName);
        });
